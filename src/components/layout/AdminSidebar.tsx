...
export interface SidebarLink {
  label: string;
  href: string;
  /** Material icon element rendered on the left of each link. */
  icon: React.ReactNode;
}

// ---------------------------------------------------------------------------
// Standard admin links (Requirements 23.5)
// ---------------------------------------------------------------------------

/**
 * Navigation links shown to all admin roles (admin and super_admin).
 * Exported so they can be referenced in tests.
 */
export const ADMIN_SIDEBAR_LINKS: Omit<SidebarLink, "icon">[] = [
...
// ---------------------------------------------------------------------------
// Super admin-only links (Requirements 23.6)
// ---------------------------------------------------------------------------

/**
 * Additional navigation links shown only to super_admin users.
 * Exported so they can be referenced in tests.
 */
export const SUPER_ADMIN_SIDEBAR_LINKS: Omit<SidebarLink, "icon">[] = [
...
// ---------------------------------------------------------------------------
// Icon map — keeps JSX out of the exported plain-object constants
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Sidebar link list — shared between desktop and mobile drawer
// ---------------------------------------------------------------------------
...
/**
 * Persistent admin panel sidebar.
 *
 * Reads the current Supabase session to determine whether the user is a
 * `super_admin`, then renders the appropriate set of navigation links.
 *
 * - All admin roles see the standard links (Dashboard → Settings).
 * - `super_admin` additionally sees links for Admins, Roles, Audit Logs,
 *   Database Backup, and Website Settings.
 *
 * This is a React Server Component — it has no client-side interactivity.
 * The mobile-collapsible behaviour is handled by `AdminSidebarDrawer` (a
 * Client Component wrapper) used in the admin layout.
 *
...
export default async function AdminSidebar() {
  ...
  const role = (user?.app_metadata?.role ?? null) as
    | "admin"
    | "super_admin"
    | null;

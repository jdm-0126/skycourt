 ...
 * GET /api/contact
 *
 * Admin-only endpoint — returns all contact messages ordered by creation
 * date (newest first).
 *
 * Steps:
 *   1. Authenticate — 401 if no session
 *   2. Authorise — role must be admin or super_admin → 403 if not
 *   3. Fetch all contact_messages via admin client (bypasses RLS)
 *   4. Return 200 { messages }
 *
 * Returns:
 *   200 — { messages: ContactMessage[] }
...
export async function GET(_request: NextRequest): Promise<NextResponse> {
  ...
  // -------------------------------------------------------------------------
  // 3. Fetch all contact messages via admin client
  // -------------------------------------------------------------------------
  const adminClient = createAdminClient();
  ...
  if (queryError) {
    console.error("[GET /api/contact] DB query error:", queryError.message);
    return NextResponse.json(
